...
export class DeployContractService {
  ...
  async deploy(cs: WalletConnectService, walletAddress: any, collectionName: string) {
    ...
    try {
      ...
      if (localStorage.getItem('chainId') == "56" || localStorage.getItem('chainId') == "97") {
        ...
        debugger
        const contract = await factory.deploy(walletAddress, this.nftArtistContract, this.ArtistMoonBoxNftSwap, collectionName, collectionName);
        ...
        const factory = new ethers.ContractFactory(abi, bytecode, cs.signer)
        debugger
        const contract = await factory.deploy(walletAddress, this.nftArtistContract, collectionName, collectionName);
        var deployAddress = await contract.deployed();
      }
